...
type postData struct {
	...
	UserName string `json:"quantity"`
	Password string `json:"buyprice"`
}
type Res struct {
...
func search(c echo.Context) error {
	...
	}else if (name=="" && password=="")||(name=="null" && password=="null"){
		err = db.Find(bson.M{"username":username}).All(&results.Data)
	}else if (name=="" && username=="")||(name=="null" && username=="null"){
		err = db.Find(bson.M{"password":password}).All(&results.Data)
	}else if (password=="")||(password=="null" ){
		err = db.Find(bson.M{"name":name,"username":username}).All(&results.Data)
	}else if (username=="")||(username=="null"){
		err = db.Find(bson.M{"name":name,"password":password}).All(&results.Data)
	}else if (name=="" )||(name=="null"){
		err = db.Find(bson.M{"username":username,"password":password}).All(&results.Data)
	}else if (name!="" && username != "" && password!= "" )||(name!="null" && username != "null" && password!= "null"){
		err = db.Find(bson.M{"name":name,"username":username,"password":password}).All(&results.Data)
	}else{
...
func put(c echo.Context)(err error){
	...
	err = db.Find(bson.M{"name": res.Data[0].Name,"username":res.Data[0].UserName,"password":res.Data[0].Password}).One(&result)
